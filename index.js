...
function transformDictionary(dictionary) {
  ...
  for (const v of dictionary.values()) {
    values.push(v);
  }
...
function getFilteredDictionarySize(dictionary, filter) {
  ...
  for (const [key, value] of dictionary.entries()) {
    ...
    filter(key, value)
      ? m.set(key, value)
      : console.log(`${key} is too small city.`);
  }
  // Повертаємо розмір фільтрованого словника, використовуючи властивість size
  return console.log(`But ${m.size} cities of list is big enough`);
}
...
function sortByValues(dictionary) {
  ...
  arr.sort((el1, el2) => el1.values() > el2.values());
